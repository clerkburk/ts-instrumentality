 ...
 *
 * @param pathorMode - The file system path (string) or mode (number) to evaluate.
 ...
 */
export function roadType(pathorMode: string | number) {
 ...
 * @property {string} {@link isAt} - The resolved absolute path that this node points to.
 * @property {string} {@link name} - The base name of the current node's path.
 ...
 * @staticmethod {@link factorySync} - Synchronously creates an instance of the appropriate {@link Road} subclass for the given path.
 * 
 * @method {@link verifySync} - Synchronously verifies that the path exists and matches the expected type.
 * @method {@link verify} - Asynchronously verifies that the path exists and matches the expected type.
 * @method {@link assertMutable} - Asserts that the node is mutable, throwing an error if not.
 * @method {@link existsSync} - Checks synchronously if the path exists and matches the expected type.
 * @method {@link exists} - Checks asynchronously if the path exists and matches the expected type.
 * @method {@link statsSync} - Gets synchronous file statistics.
 ...
 * @method {@link accessibleSync} - Checks synchronously if the path is accessible with the given mode.
 * @method {@link accessible} - Checks asynchronously if the path is accessible with the given mode.
 * @method {@link untilAccessible} - Waits asynchronously until the path becomes accessible, with abort and callback support.
 * @method {@link onChange} - Watches for changes to the path, with abort and callback support.
 * 
 * @abstractmethod {@link deleteSync} - Synchronously deletes the path.
 * @abstractmethod {@link delete} - Asynchronously deletes the path.
 * @abstractmethod {@link moveSync} - Synchronously moves the path into a folder.
 * @abstractmethod {@link move} - Asynchronously moves the path into a folder.
 * @abstractmethod {@link copySync} - Synchronously copies the path into a folder.
 * @abstractmethod {@link copy} - Asynchronously copies the path into a folder.
 * @abstractmethod {@link renameSync} - Synchronously renames the path.
...
export abstract class Road {
   ...
   *
   * @param lookFor - The file system path to check for existence and resolve.
   ...
   *
   * @param lookFor - The file path to check and use for instantiating the {@link Road} subclass.
  ...
  static async factory(lookFor: string) {
    ...
    const roadCtor = roadType(lookFor)
    return new roadCtor(lookFor)
  }
  /**
   * Synchronously creates an instance of a {@link Road} subclass based on the provided file path.
   *
   * This factory method first checks if the specified file exists, determines the appropriate
   * {@link Road} subclass constructor using the {@link roadType} function, and then returns a new instance
   * of that subclass initialized with the given file path.
   *
   * @param lookFor - The file path to check and use for instantiating the {@link Road} subclass.
   * @returns An instance of a {@link Road} subclass.
   * @throws If the file does not exist or if instantiation fails.
   */
   ...
   *
   * @param mode - The accessibility checks to be performed (default is `fs.constants.F_OK`).
   ...
   */
  accessibleSync(mode: number = fs.constants.F_OK) {
    try {
   ...
   *
   * @param mode - The accessibility checks to perform (defaults to `fs.constants.F_OK`).
   ...
   */
  async accessible(mode: number = fs.constants.F_OK) {
    try {
   ...
   *
   * @param mode - The accessibility check mode (e.g., `fs.constants.F_OK`, `fs.constants.R_OK`). Defaults to `fs.constants.F_OK`.
   * @param abortSignal - An `AbortSignal` to allow aborting the wait operation.
   * @param onEachAttempt - Optional callback invoked after each unsuccessful accessibility check.
   * @returns A promise that resolves when the target becomes accessible or rejects if aborted.
   */
  async untilAccessible(mode: number = fs.constants.F_OK, abortSignal: AbortSignal, onEachAttempt?: () => unknown) {
    const watcher = fs.watch(this.isAt)
    try {
      if (await this.accessible(mode))
        return
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _ of on(watcher as any, 'change', { signal: abortSignal }))
        if (await this.accessible(mode))
          return
        else
          await onEachAttempt?.()
   ...
   *
   * @param abortSignal - An `AbortSignal` used to cancel the watcher and stop listening for changes.
   * @returns A `Promise` that resolves when the watcher is closed, either due to an abort signal or after completion.
   * @async
   */
  async onChange(abortSignal: AbortSignal) {
    const watcher = fs.watch(this.isAt)
    try {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _ of on(watcher as any, 'change', { signal: abortSignal })) // TODO: fix type
        return
    } finally {
      watcher.close()
    }
...
/**
 * Class representing a file in the filesystem, extending the {@link Road} base class.
 * 
 * @method {@link create} - Asynchronously creates a file at the specified path if it does not exist.
 * @method {@link createSync} - Synchronously creates a file at the specified path if it does not exist.
 * @method {@link readSync} - Synchronously reads the file content as a Buffer or string.
 * @method {@link read} - Asynchronously reads the file content as a Buffer or string.
 * @method {@link itBuff} - Asynchronously iterates over the file content in chunks as Buffers.
 * @method {@link itLines} - Asynchronously iterates over the lines of the file.
 * @method {@link writeSync} - Synchronously writes data to the file, overwriting existing content.
 * @method {@link write} - Asynchronously writes data to the file, overwriting existing content.
 * @method {@link appendSync} - Synchronously appends data to the end of the file.
 * @method {@link append} - Asynchronously appends data to the end of the file.
 * @method {@link readStream} - Creates a readable stream for the file.
 * @method {@link writeStream} - Creates a writable stream for the file.
 * @method {@link sameAs} - Asynchronously compares the content and position of this file with another file.
 * @method {@link sameAsSync} - Synchronously compares the content and position of this file with another file.
 * @property {@link ext} - Returns the file extension of the current path.
 */
export class File extends Road {
   ...
   *
   * @param at - The file path where the `File` should be created.
   ...
   * @param flag - The file system flag.
   ...
   * 
   * @param chunkSize - The size of each chunk to read from the file (default is 64KB).
   * @param flags - The file system flags to use when opening the file.
   * @yields {Promise<Buffer>} A promise that resolves to each chunk of bytes read from the file.
   * @throws {Error} If the file cannot be read, an error is thrown.
   */
  async *itBuff(chunkSize: number = 64 * 1024, flags?: string) {
    const fd = await fp.open(this.isAt, flags)
    try {
   ...
   * 
   * @param options - The options for reading the file, including encoding and flags (default is `{ encoding: 'utf-8' }`).
   * @yields {Promise<string>} A promise that resolves to each line of the file as a string.
   * @throws {Error} If the file cannot be read, an error is thrown.
   */
  async *itLines(options: BufferEncoding | fs.ReadStreamOptions = { encoding: 'utf-8' }) {
   ...
   * 
   * @param data - The data (Buffer or string) to write to the file.
   * @param options - The options for writing the file, including encoding and flags.
  ...
  async write(data: Buffer | string, options?: fs.WriteFileOptions) {
    this.assertMutable()
    return fp.writeFile(this.isAt, data, options)
  }
  /**
   * Appends the provided data to the file at `this.isAt` synchronously.
   * 
   * @param data - The data (Buffer or string) to append to the file.
   * @param options - The options for appending to the file, including encoding and flags.
  ...
  async append(data: Buffer | string, options?: fs.WriteFileOptions) {
    this.assertMutable()
    return fp.appendFile(this.isAt, data, options)
  }
   ...
   *
   * @param other - The other `File` instance to compare against.
   ...
   */
  async sameAs(other: File): Promise<boolean> {
    if ((await fp.lstat(this.isAt)).size !== (await fp.lstat(other.isAt)).size)
      return false
    const thisBuffer = await fp.readFile(this.isAt)
    const otherBuffer = await fp.readFile(other.isAt)
  ...
  sameAsSync(other: File): boolean {
    if (fs.statSync(this.isAt).size !== fs.statSync(other.isAt).size)
      return false
    const thisBuffer = fs.readFileSync(this.isAt)
    const otherBuffer = fs.readFileSync(other.isAt)
    return thisBuffer.equals(otherBuffer)
  }
   ...
   *
   * @param into - The target folder where the file should be moved.
   * @throws {Error} If the node is not mutable.
   */
   ...
   * @throws {Error} If the node is not mutable.
   */
  async move(into: Folder): Promise<void> {
    this.assertMutable()
    const newPath = into.join(this.name)
   ...
   *
   * @param into - The destination folder where the file will be copied.
   ...
   *
   * @param into - The destination folder where the file should be copied.
   ...
   * by joining the parent directory with the provided {@link to} string, and
   * then renames the file or directory on the filesystem. After renaming,
   * it updates the internal reference to the new path.
   *
   * @param to - The new name or relative path for the node.
   ...
   *
   * @param to - The new name for the node.
 ...
 *
 * @function {@link create}: Asynchronously creates a folder at the specified path if it does not exist.
 * @function {@link createSync}: Synchronously creates a folder at the specified path if it does not exist.
 * @method {@link listSync}: Synchronously lists entries in the folder, optionally filtering by type.
 * @method {@link list}: Asynchronously lists entries in the folder, optionally filtering by type.
 * @method {@link findSync}: Synchronously finds an entry by name, optionally filtering by type.
 * @method {@link find}: Asynchronously finds an entry by name, optionally filtering by type.
 * @method {@link delete_sync}: Synchronously deletes the folder and its contents.
 * @method {@link delete}: Asynchronously deletes the folder and its contents.
 * @method {@link move_sync}: Synchronously moves the folder into another folder.
 * @method {@link move}: Asynchronously moves the folder into another folder.
 * @method {@link copy_sync}: Synchronously copies the folder into another folder.
 * @method {@link copy}: Asynchronously copies the folder into another folder.
 * @method {@link rename_sync}: Synchronously renames the folder.
 * @method {@link rename}: Asynchronously renames the folder.
 *
 * @remarks
...
export class Folder extends Road {
   ...
   *
   * @param paths - One or more string path segments to join to the current path.
   ...
   * 
   * @param options - The options for deletion, such as `recursive` (default is `true`).
   * @throws {Error} If the node is not mutable or if the deletion fails.
   */
  deleteSync(options: fs.RmOptions = { recursive: true }): void {
    this.assertMutable()
   ...
   */
  async delete(options: fs.RmOptions = { recursive: true }): Promise<void> {
    this.assertMutable()
   ...
   *
   * @param into - The target `Folder` instance where this node should be moved.
   ...
   *
   * @param into - The target `Folder` to move this node into.
   ...
   *
   * @param into - The target `Folder` instance where the current folder will be copied.
   ...
   *
   * @param into - The destination `Folder` where the current folder will be copied.
   ...
   *
   * @param to - The new name or path for the node relative to its parent.
   * @throws {Error} If the node is not mutable or if the rename operation fails.
   */
  renameSync(to: string): void {
    this.assertMutable()
...
export class SymbolicLink extends Road {
   ...
   *
   * @param at - The file system path where the symbolic link should be created.
   * @param target - (Optional) The target `Road` object to which the symbolic link should point.
   * @returns A promise that resolves to a new `SymbolicLink` instance representing the created or existing link.
   */
  static async create(at: string, target?: Road): Promise<SymbolicLink> {
    try {
      await fp.access(at, fs.constants.F_OK)
    } catch {
 ...
 * @remarks This class extends {@link UnusableRoad} and inherits its properties and methods.
 */
  ...
  constructor(_at: string) {
    ...
    ;(async () => {
      while (!this.abortController.signal.aborted) {
        await this.update()
        await this.onChange(this.abortController.signal)
      }
    })()
  }
  update_sync(): void {
    this.lastReadContent = this.readSync()
  }
...
export class TempFile extends File implements AsyncDisposable, Disposable {
  ...
  constructor() {
    super(File.createSync(ph.join(os.tmpdir(), `tempfile_${Date.now()}_${crypto.randomUUID()}.tmp`)).isAt)
    ;['exit', 'SIGINT', 'SIGTERM'].forEach(event => {
      process.on(event, () => {
        try {
          this.deleteSync()
        } catch (e) {
          console.error(`Failed to delete temp file at '${this.isAt}' on process ${event}:`, e)
        }
      })
    })
  }
...
export class TempFolder extends Folder implements AsyncDisposable, Disposable {
  override readonly mutable: boolean = true // Must be mutable to allow deletion
  constructor() {
    super(Folder.createSync(ph.join(os.tmpdir(), `tempfolder_${Date.now()}_${crypto.randomUUID()}`)).isAt)
    ;['exit', 'SIGINT', 'SIGTERM'].forEach(event => {
      process.on(event, () => {
        try {
          this.deleteSync()
        } catch (e) {
          console.error(`Failed to delete temp folder at '${this.isAt}' on process ${event}:`, e)
        }
      })
    })
  }
